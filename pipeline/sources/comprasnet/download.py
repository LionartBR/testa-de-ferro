...
# Dados Abertos portal and save them to the raw directory.
#
# Design decisions:
#   - Comprasnet files are served as direct CSV downloads from
#     dadosabertos.compras.gov.br — no ZIP wrapping, unlike the Receita
...
#   - follow_redirects is enabled because the Dados Abertos portal uses HTTP
#     redirects to route requests to storage backends.
...
def download_comprasnet(url: str, raw_dir: Path, timeout: int = 300) -> Path:
    """Download a Comprasnet CSV file from Dados Abertos and save it locally.
